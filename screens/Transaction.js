...
export default class TransactionScreen extends Component {
	...
	checkBookAvailability = async (bookId) => {
		...
		let bookRef = await getDocs(dbQuery); //JSON---> {key:value} --> Object.keys()--> [key1, key2,...]
//condition ? true statement : false statement
	...
	checkStudentEligibilityForBookIssue = async (studentId) => {
		var studentQuery = query(collection(db,'students'),where('student_id','==',studentId))
		var studentRef = await getDocs(studentQuery)
		...
		if(studentRef.docs.length == 0){
			...
			studentRef.forEach((docs)=>{
				if(docs.data().number_of_books_issued < 2){
					isStudentEligible = true;
				}
				else{
					isStudentEligible = false;
			        alert("This student already took the maximum amount of books.")
					this.setState({bookId:"",studentId:""})
				}
			})
		}
